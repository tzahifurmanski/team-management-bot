...
export const getAskChannelStatsForYesterday = async function () {
  ...
  await sendSlackMessage(
    `In the last ${DAYS_BACK}, <#${TEAM_ASK_CHANNEL_ID}> had a *total of ${monthStats.totalMessages} asks*. Out of those, *${monthStats.totalNumProcessed} were answered*, *${monthStats.totalNumInProgress} are in progress*, and *${monthStats.totalNumUnchecked} were not handled*.`,
    TEAM_CHATTER_CHANNEL_ID
  );
};
