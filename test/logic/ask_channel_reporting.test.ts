import {AsksChannelReportResult, createReport, createReportSection} from "../../src/logic/ask_channel_reporting";
...
const mockedUserProfile = getUserProfile as jest.MockedFunction<typeof getUserProfile>;
...
describe("createReport", () => {
    ...
    test("One user with a long names", async () => {
        ...
        const mockedUser = { id: "<USER_ID>", division: "ZIGI Division is the best division",
            department: "ZIGI Department is the best department", team: "TEAM ZIGI is the best team" } as MockedUser;
        ...
        const userProfile : any = getProfileForTest(mockedUser.team, mockedUser.division, mockedUser.department);
    ...
    test("Four users from 2 divisions, 3 departments, 4 teams", async () => {
        ...
        const mockedUserA = { id: "<USER_ID1>", division: "A", department: "A", team: "A" } as MockedUser;
        // User B - Division A, Department B, Team B
        const mockedUserB = { id: "<USER_ID2>", division: "A", department: "B" , team: "B"} as MockedUser;
        // User C - Division B, Department C, Team C
        const mockedUserC = { id: "<USER_ID3>", division: "B", department: "C" , team: "C"} as MockedUser;
        // User D - Division B, Department C, Team D
        const mockedUserD = { id: "<USER_ID4>", division: "B", department: "C" , team: "D"} as MockedUser;

        const messages : any[] = [ getMessageForTest(mockedUserA.id), getMessageForTest(mockedUserB.id),
            getMessageForTest(mockedUserC.id), getMessageForTest(mockedUserD.id)];

        // Mock returned profile
        mockedUserProfile.mockResolvedValueOnce(getProfileForTest(mockedUserA.team, mockedUserA.division, mockedUserA.department));
        mockedUserProfile.mockResolvedValueOnce(getProfileForTest(mockedUserB.team, mockedUserB.division, mockedUserB.department));
        mockedUserProfile.mockResolvedValueOnce(getProfileForTest(mockedUserC.team, mockedUserC.division, mockedUserC.department));
        mockedUserProfile.mockResolvedValueOnce(getProfileForTest(mockedUserD.team, mockedUserD.division, mockedUserD.department));
...
describe("createReportSection", () => {
    ...
    const validateSection = ((block : SectionBlock, text : string, fields : any[]) => {
